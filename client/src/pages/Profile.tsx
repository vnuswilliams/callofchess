...
import { Award, BarChart3, BookOpen, Check, CheckCircle2, Clock3, Copy, LockKeyhole, LogIn, RefreshCw, Share2, Trophy } from "lucide-react";
...
import { LEARNING_PATH_PROGRESS_UPDATED_EVENT, normalizeProgressLessonIds } from "@/lib/learningPathProgress";
import { mergeRealtimeProgressRows, type RealtimeProgressPayload } from "@/lib/learningPathRealtime";
import { copyText, isShareAbortError, shareOrCopy } from "@/lib/share";
...
function lessonName(id: string, language: "fr" | "en", t: (key: string) => string) {
  return lessonNames[id]?.[language] ?? t("inline_646141d3f2");
}
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const { data, error: progressError } = await supabase.from("lesson_progress").select("lesson_id, completed_steps, completed, updated_at").eq("user_id", auth.user.id).order("updated_at", { ascending: false }).limit(30);
      if (!active) return;
      if (progressError) setError(true); else setRows(normalizeProgressLessonIds((data ?? []) as ProgressRow[]));
      setLoading(false);
      ...
      if (!progressChannel || channelUserId !== auth.user.id) {
        ...
        progressChannel = supabase
          .channel(`profile-progress:${auth.user.id}`)
          .on("postgres_changes", {
            event: "*",
            schema: "public",
    ...
    const refreshProgress = () => { void load().catch(() => { if (active) { setError(true); setLoading(false); } }); };
    const { data: listener } = supabase.auth.onAuthStateChange(() => { refreshProgress(); });
    void load().catch(() => { if (active) { setError(true); setRealtimeStatus("fallback"); setLoading(false); } });
  ...
  async function shareProfile() {
    ...
    try {
      setShareState(await shareOrCopy(shareData));
    } catch (shareError) {
  ...
  async function copyProfileLink() {
    try { await copyText(`${window.location.origin}/profile`); setShareState("copied"); } catch { setShareState("error"); }
  }
  ...
  async function shareBadge() {
    ...
    try {
      setShareState(await shareOrCopy({ title: copy.title, text: copy.description, url }));
    } catch (shareError) {
      if (isShareAbortError(shareError)) return;
      setShareState("error");
    }
