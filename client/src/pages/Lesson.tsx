          ...
          <aside className="space-y-4">
            ...
            <section className="lesson-analysis border border-[#cbbd99] bg-[#173e37] p-5 text-[#fffaf0]">
              ...
              {showAnalysis && <div className="mt-5 border-t border-[#496d61] pt-4" aria-live="polite">
                ...
                {analysis && <div className="space-y-4 text-sm"><div className="rounded-[.65rem] border border-[#66857c] bg-[#21473e] p-4"><p className="text-[.62rem] font-extrabold uppercase tracking-[.12em] text-[#e7ba61]">{beginnerExplanation?.label}</p><p className="mt-2 text-base font-semibold leading-6 text-[#fffaf0]">{beginnerExplanation?.summary}</p><div className="mt-3 border-t border-[#496d61] pt-3"><p className="text-[.62rem] font-extrabold uppercase tracking-[.12em] text-[#9cb4a9]">{language === "fr" ? "Pourquoi ?" : "Why?"}</p><p className="mt-1 leading-6 text-[#d9e0d6]">{beginnerExplanation?.why}</p></div><div className="mt-3 border-t border-[#496d61] pt-3"><p className="text-[.62rem] font-extrabold uppercase tracking-[.12em] text-[#e7ba61]">{language === "fr" ? "La question à se poser" : "Question to ask yourself"}</p><p className="mt-1 font-semibold leading-6 text-[#fffaf0]">{beginnerExplanation?.nextQuestion}</p></div></div><details className="rounded-[.65rem] border border-[#496d61] p-3"><summary className="cursor-pointer text-xs font-extrabold uppercase tracking-[.1em] text-[#cbd8cc]">{language === "fr" ? "Voir les détails Stockfish" : "View Stockfish details"}</summary><div className="mt-3 grid grid-cols-2 gap-3"><div><span className="block text-[.6rem] font-extrabold uppercase tracking-[.12em] text-[#9cb4a9]">{t("evaluation")}</span><strong className="mt-1 block font-mono text-xl text-[#e7ba61]">{analysis.scoreLabel}</strong></div><div><span className="block text-[.6rem] font-extrabold uppercase tracking-[.12em] text-[#9cb4a9]">{t("depth")}</span><strong className="mt-1 block font-mono text-xl text-[#fffaf0]">{analysis.depth || "—"}</strong></div><div className="col-span-2"><span className="block text-[.6rem] font-extrabold uppercase tracking-[.12em] text-[#9cb4a9]">{t("bestMove")}</span><strong className="mt-1 block font-mono text-lg text-[#fffaf0]">{analysis.bestMove ? formatUciAsSan(position, analysis.bestMove) : t("waitingAnalysis")}</strong></div><div className="col-span-2"><span className="block text-[.6rem] font-extrabold uppercase tracking-[.12em] text-[#9cb4a9]">{t("mainLine")}</span><p className="mt-1 font-mono text-xs leading-6 text-[#d9e0d6]">{analysis.principalVariation.length ? formatPrincipalVariation(position, analysis.principalVariation).join(" · ") : t("waitingAnalysis")}</p></div></div></details></div>}
              </div>}
