  ...
  return (
    <section className="lesson-coaching border border-[#d6a16b] bg-[#fff1dc] p-5" aria-live="polite">
      <div className="flex items-start gap-3">
        ...
        <div><p className="eyebrow text-[#9a6b18]">{t("personalized")} · {t("inline_4a19c30b1b")} {mistake.attemptNumber}</p><h2 className="display-font mt-2 text-3xl leading-none text-[#173e37]">{mistake.title}</h2></div>
      </div>
      <p className="mt-4 text-sm leading-6 text-[#4e5146]">{mistake.explanation}</p>
      <div className="mt-4 border-l-2 border-[#d69024] pl-4"><p className="text-[.62rem] font-extrabold uppercase tracking-[.12em] text-[#9a6b18]">{t("inline_226c16bea8")}</p><p className="mt-1 text-sm font-semibold leading-6 text-[#3c4c43]">{mistake.recommendation}</p></div>
...
export default function Lesson() {
  ...
  const beginnerExplanation = useMemo(() => analysis ? explainEngineForBeginner(analysis.scoreCp, analysis.mate, analysis.bestMove ? formatUciAsSan(position, analysis.bestMove) : null, language, mistake) : null, [analysis, language, mistake, position]);
      ...
      <main className="mx-auto max-w-[1440px] px-5 pb-20 pt-10 sm:px-8 lg:px-12 lg:pt-14">
        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1.23fr)_390px] xl:gap-12">
          ...
          <aside className="space-y-4">
            <section className="lesson-paper border border-[#cbbd99] bg-[#fffaf0] p-6 sm:p-7"><div className="flex items-center justify-between"><span className="eyebrow">{t("inline_28d9964386")}</span><span className="font-mono text-xs font-bold text-[#a87416]">{completed ? "✓" : `0${currentStep + 1}`}</span></div>{completed ? <div className="mt-6"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#d69024] text-[#173e37]"><Trophy size={22} /></div><h2 className="display-font mt-5 text-4xl leading-none tracking-[-.04em] text-[#173e37]">{t("inline_db305833f7")}</h2><p className="mt-4 leading-7 text-[#5f5b4e]">{t("inline_9eba926421")}</p></div> : <div className="mt-6"><h2 className="display-font text-4xl leading-none tracking-[-.04em] text-[#173e37]">{activeStepCopy.answer}</h2><p className="mt-5 leading-7 text-[#5f5b4e]">{activeStepCopy.idea}</p></div>}
              ...
              {completed && <div className="mt-7 space-y-2">
                {nextLessonId ? <a href={`/lesson/${nextLessonId}`} className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-[.65rem] bg-[#d69024] px-4 text-center text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#173e37] transition-colors hover:bg-[#e7ba61]"><ChevronRight size={15} /> {t("lessons.nextLesson")}</a> : <a href="/path" className="inline-flex min-h-10 w-full items-center justify-center gap-2 rounded-[.65rem] bg-[#d69024] px-4 text-center text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#173e37] transition-colors hover:bg-[#e7ba61]"><ChevronRight size={15} /> {t("lessons.viewPath")}</a>}
                <Button onClick={resetLesson} variant="outline" className="w-full rounded-[.65rem] border-[#bcae88] bg-transparent text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#173e37] hover:bg-[#efe4cb]"><RotateCcw size={15} /> {t("inline_3943304b7d")}</Button>
              </div>}
            </section>

            <section className={`lesson-feedback border p-5 ${feedback === "wrong" ? "border-[#c96442] bg-[#fff0e7]" : feedback === "complete" ? "border-[#6f977c] bg-[#e9f0e6]" : feedback === "correct" ? "border-[#90a98d] bg-[#f2f4e9]" : "border-[#cbbd99] bg-[#f5ecd8]"}`}><div className="flex gap-3"><div className={`mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full ${feedback === "wrong" ? "bg-[#c96442] text-white" : "bg-[#d69024] text-[#173e37]"}`}>{feedback === "wrong" ? <CircleHelp size={15} /> : <Check size={16} strokeWidth={3} />}</div><div><p className="text-[.65rem] font-extrabold uppercase tracking-[.13em] text-[#736954]">{feedback === "wrong" ? t("tryAgain") : feedback === "idle" ? (t("inline_901c5496b0")) : feedback === "complete" ? (t("inline_4b77dd777e")) : (t("inline_85aa755eb6"))}</p><p className="mt-2 text-sm leading-6 text-[#4e5146]">{feedback === "wrong" ? t("wrongMove") : feedback === "complete" ? (t("inline_cb1df634ec")) : feedback === "correct" ? (t("inline_295297f9e2")) : t("advice")}</p></div></div></section>
            ...
            <Card className="lesson-analysis rounded-xl border-[#cbbd99] bg-[#173e37] p-5 text-[#fffaf0] shadow-sm">
              ...
              {showAnalysis && <div className="mt-5 border-t border-[#496d61] pt-4" aria-live="polite">
                ...
                {!engineError && !analysis && <p className="flex items-center gap-2 text-sm text-[#d9e0d6]"><Loader2 size={15} className="animate-spin" /> {t("inline_6537493693")}</p>}
        ...
        <section className="lesson-notes mt-10 grid gap-px border border-[#cbbd99] bg-[#cbbd99] md:grid-cols-3"><div className="bg-[#fffaf0] p-6"><span className="font-mono text-xs font-bold text-[#d69024]">01 /</span><h3 className="display-font mt-5 text-3xl text-[#173e37]">{t("noteCenter")}</h3><p className="mt-3 text-sm leading-6 text-[#615d50]">{t("inline_6c73271b11")}</p></div><div className="bg-[#fffaf0] p-6"><span className="font-mono text-xs font-bold text-[#d69024]">02 /</span><h3 className="display-font mt-5 text-3xl text-[#173e37]">{t("noteDevelopment")}</h3><p className="mt-3 text-sm leading-6 text-[#615d50]">{t("inline_7c3c62986d")}</p></div><div className="bg-[#fffaf0] p-6"><span className="font-mono text-xs font-bold text-[#d69024]">03 /</span><h3 className="display-font mt-5 text-3xl text-[#173e37]">{t("noteSafety")}</h3><p className="mt-3 text-sm leading-6 text-[#615d50]">{t("inline_989727d6c3")}</p></div></section>
      </main>
    </div>
  );
}
