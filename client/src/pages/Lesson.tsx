      ...
      <main className="mx-auto max-w-[1440px] px-5 pb-20 pt-10 sm:px-8 lg:px-12 lg:pt-14">
        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1.23fr)_390px] xl:gap-12">
          ...
          <aside className="space-y-4">
            <section className="lesson-paper border border-[#cbbd99] bg-[#fffaf0] p-6 sm:p-7"><div className="flex items-center justify-between"><span className="eyebrow">{t("inline_28d9964386")}</span><span className="font-mono text-xs font-bold text-[#a87416]">{completed ? "✓" : `0${currentStep + 1}`}</span></div>{completed ? <div className="mt-6"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#d69024] text-[#173e37]"><Trophy size={22} /></div><h2 className="display-font mt-5 text-4xl leading-none tracking-[-.04em] text-[#173e37]">{t("inline_db305833f7")}</h2><p className="mt-4 leading-7 text-[#5f5b4e]">{lesson.solution[copy]}</p></div> : <div className="mt-6"><h2 className="display-font text-4xl leading-none tracking-[-.04em] text-[#173e37]">{activeStep.answer[copy]}</h2><p className="mt-5 leading-7 text-[#5f5b4e]">{activeStep.idea[copy]}</p></div>}{!completed && <Button variant="outline" onClick={() => setShowHint((value) => !value)} className="mt-6 min-h-11 w-full rounded-[.65rem] border-[#bcae88] bg-transparent text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#173e37] hover:bg-[#efe4cb]"><Lightbulb size={15} /> {showHint ? t("hideHint") : t("hint")}</Button>}{completed && <div className="mt-7 space-y-2"><a href={lessonKey === "6" ? "/path" : `/lesson/${PUBLIC_LESSON_ID_BY_KEY[String(Number(lessonKey) + 1) as keyof typeof PUBLIC_LESSON_ID_BY_KEY]}`} className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-[.65rem] bg-[#d69024] px-4 text-center text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#173e37] transition-colors hover:bg-[#e7ba61]"><ChevronRight size={15} /> {lessonKey === "6" ? t("lessons.viewPath") : t("lessons.nextLesson")}</a><Button onClick={resetLesson} variant="outline" className="min-h-11 w-full rounded-[.65rem] border-[#bcae88] bg-transparent text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#173e37] hover:bg-[#efe4cb]"><RotateCcw size={15} /> {t("inline_3943304b7d")}</Button></div>}</section>
            <section className={`lesson-feedback border p-5 ${feedback === "wrong" ? "border-[#c96442] bg-[#fff0e7]" : feedback === "complete" ? "border-[#6f977c] bg-[#e9f0e6]" : feedback === "correct" ? "border-[#90a98d] bg-[#f2f4e9]" : "border-[#cbbd99] bg-[#f5ecd8]"}`} aria-live="polite"><div className="flex gap-3"><div className={`mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full ${feedback === "wrong" ? "bg-[#c96442] text-white" : "bg-[#d69024] text-[#173e37]"}`}>{feedback === "wrong" ? <CircleHelp size={15} /> : <Check size={16} strokeWidth={3} />}</div><div><p className="text-[.65rem] font-extrabold uppercase tracking-[.13em] text-[#736954]">{feedbackTitle}</p><p className="mt-2 text-sm leading-6 text-[#4e5146]">{feedbackText}</p></div></div></section>
            <section className="lesson-history rounded-xl border border-[#cbbd99] bg-[#ece0c1] p-5"><p className="eyebrow">{t("moveSheet")}</p><div className="mt-4 min-h-13 border-l border-[#bfae83] pl-4 font-mono text-sm font-bold leading-7 text-[#28483f]">{history.length ? history.map((move) => <div key={move}>{move}</div>) : <span className="text-[#867c64]">{t("waitingMove")}</span>}</div>{completed && <div className="mt-4 flex items-center gap-2 border-t border-[#c7b48a] pt-4 text-[.65rem] font-extrabold uppercase tracking-[.12em] text-[#6c725c]"><Check size={14} className="text-[#467a5d]" /> {t("objectiveFilled")}</div>}</section>
          </aside>
        </div>
