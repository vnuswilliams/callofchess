        ...
        <div className="grid gap-8 xl:grid-cols-[minmax(0,1.23fr)_390px] xl:gap-12">
          ...
          <aside className="space-y-4">
            <section className="lesson-paper border border-[#cbbd99] bg-[#fffaf0] p-6 sm:p-7"><div className="flex items-center justify-between"><span className="eyebrow">{t("inline_28d9964386")}</span><span className="font-mono text-xs font-bold text-[#a87416]">{completed ? "✓" : `0${currentStep + 1}`}</span></div>{completed ? <div className="mt-6"><div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#d69024] text-[#173e37]"><Trophy size={22} /></div><h2 className="display-font mt-5 text-4xl leading-none tracking-[-.04em] text-[#173e37]">{t("inline_db305833f7")}</h2><p className="mt-4 leading-7 text-[#5f5b4e]">{t("inline_9eba926421")}</p></div> : <div className="mt-6"><h2 className="display-font text-4xl leading-none tracking-[-.04em] text-[#173e37]">{activeStepCopy.answer}</h2><p className="mt-5 leading-7 text-[#5f5b4e]">{activeStepCopy.idea}</p></div>}
              ...
              {completed && <Button onClick={resetLesson} className="mt-7 w-full rounded-[.65rem] bg-[#173e37] text-[.67rem] font-extrabold uppercase tracking-[.1em] text-[#fffaf0] hover:bg-[#285448]"><RotateCcw size={15} /> {t("inline_3943304b7d")}</Button>}
            </section>
