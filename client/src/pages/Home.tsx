...
export default function Home() {
  ...
  const methodSteps = [
    { icon: "01", title: t("common.landing.method.idea"), text: t("common.landing.method.ideaText") },
    { icon: "02", title: t("common.landing.method.position"), text: t("common.landing.method.positionText") },
    { icon: "03", title: t("common.landing.method.reflex"), text: t("common.landing.method.reflexText") },
  ];
      ...
      <header className="echequier-header paper-texture relative z-30 border-b border-[#cbc09f]">
        <div className="mx-auto flex min-h-[76px] max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12">
          ...
          <nav className="hidden items-center gap-8 text-[.7rem] font-extrabold uppercase tracking-[.12em] lg:flex" aria-label="Navigation principale">
            <Link className="transition-colors hover:text-[#8b6217]" href="/path">{t("inline_8ce4187b64")}</Link>
        ...
        <section id="parcours" className="echequier-path relative bg-[#fffaf0] py-24 sm:py-32">
          <div className="mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12">
            ...
            <div className="mt-12 grid gap-px bg-[#d3c7aa] md:grid-cols-3">
              {pathItems.map((item, index) => <Link key={item.n} href={item.href} aria-label={`${item.n} — ${item.title}`} className={`group relative flex min-h-[310px] flex-col bg-[#fffaf0] p-7 transition-colors hover:bg-[#f5ecd7] focus-visible:z-10 sm:p-9 ${index === 1 ? "md:translate-y-8" : ""}`}><span className="font-mono text-xs font-bold text-[#d69024]">{item.n} /</span><h3 className="display-font mt-9 text-[2.35rem] leading-none tracking-[-.04em] text-[#173e37]">{item.title}</h3><p className="mt-5 max-w-[27ch] text-sm leading-6 text-[#625d50]">{item.text}</p><div className="mt-auto flex items-center justify-between pt-8"><span className="text-[.65rem] font-extrabold uppercase tracking-[.14em] text-[#756d58]">{item.tag}</span><span className="grid h-11 w-11 place-items-center border border-[#b9ab86] transition-all group-hover:border-[#d69024] group-hover:bg-[#d69024]" aria-hidden="true"><ArrowUpRight size={15} /></span></div></Link>)}
            </div>
          </div>
        </section>
