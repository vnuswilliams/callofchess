...
export default function ComputerLesson({ lesson }: { lesson: LessonDefinition }) {
  ...
  const lastBoardInteraction = useRef<{ square: string; at: number } | null>(null);
  ...
  const reset = () => {
    ...
    lastBoardInteraction.current = null;
  };
  ...
  const handleBoardInteraction = (square: string) => {
    const now = Date.now();
    if (lastBoardInteraction.current?.square === square && now - lastBoardInteraction.current.at < 80) {
      lastBoardInteraction.current = null;
      return;
    }
    lastBoardInteraction.current = { square, at: now };
