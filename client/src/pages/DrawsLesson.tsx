...
export default function DrawsLesson({ lesson }: { lesson: LessonDefinition }) {
  ...
  const lastSpecialInteraction = useRef<{ square: string; at: number } | null>(null);
  ...
  const handleSpecialBoardInteraction = (square: string) => {
    const now = Date.now();
    if (lastSpecialInteraction.current?.square === square && now - lastSpecialInteraction.current.at < 80) {
      lastSpecialInteraction.current = null;
      return;
    }
    lastSpecialInteraction.current = { square, at: now };
    handleSpecialSquareClick(square);
  };
