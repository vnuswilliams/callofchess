...
export function friendlyAuthError(error: unknown, fr: boolean) {
  ...
  if (text.includes("rate limit") || text.includes("rate_limit") || text.includes("too many requests")) return fr ? "Trop de tentatives rapprochées. Attendez quelques instants puis réessayez." : "Too many attempts in a short time. Wait a moment and try again.";
