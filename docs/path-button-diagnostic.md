...
L’historique Vercel du projet `callofchess` affiche également le commit documentaire `ed5ff35` « Document path lesson resume verification » en environnement **Production**, sur `main`, juste après le commit fonctionnel `2356993`. La page de déploiements confirmait son entrée récente ; la vue détaillée n’a pas pu être réouverte après le changement de page, mais l’entrée textuelle est bien présente.
